<?php include 'vendor/connect.php';
  if (isset($_GET['id'])){
    $sql = $link->query("SELECT * FROM `players` WHERE `team_id` = ".$_GET['id']);
  }
  if (!$sql){
    echo '<br>';
...
<body>
  <div class="players">
    <div class="players__container container">
      <div class="team__row">
        <h1 class="team__title title">Игроки
            ...
            while ($data = $sql->fetch_assoc()) { ?>
              <tr>
                ...
                <td class="team-table__link" colspan="2">
                  <a target='_blank' href="<?= $data['player_link'] ?>">
                    <div class="team-table__img img">
                      <img alt="team icon" src="<?= $data['img_link'] ?>">
                    </div>
                    <p><?= $data['name'] ?></p>
                  </a>
                </td>
